...
const buscarCategoriaPorId = async (req, res) => {
  try {
    ...
    if (!categoria) return res.status(404).json({ message: 'Categoria nao encontrada' });
...
const criarCategoria = async (req, res) => {
  try {
    const { nome } = req.body;
    if (!nome) return res.status(400).json({ message: 'Nome da categoria e obrigatorio' });
...
const atualizarCategoria = async (req, res) => {
  try {
    ...
    if (!categoriaAtual) return res.status(404).json({ message: 'Categoria nao encontrada' });
...
const deletarCategoria = async (req, res) => {
  try {
    ...
    if (!removed) return res.status(404).json({ message: 'Categoria nao encontrada' });
